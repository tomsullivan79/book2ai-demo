...
function pickObj(obj: UnknownRec, key: string): UnknownRec {
  const v = obj[key];
  return isObj(v) ? (v as UnknownRec) : {};
}
...
/* ---------- normalizer that accepts snake_case & camelCase ---------- */
function normalizeInsights(raw: unknown): Insights {
  const root: UnknownRec = isObj(raw) ? raw : {};

  // totals (support {totals:{all_time,last_7_days}} or {totals:{all,last7}})
  const totalsObj = pickObj(root, 'totals');
  const totals = {
    all_time:
      toNumber(pick(totalsObj, 'all_time'), NaN) ||
      toNumber(pick(totalsObj, 'all'), NaN) ||
      toNumber(pick(root, 'all_time')),
    last_7_days:
      toNumber(pick(totalsObj, 'last_7_days'), NaN) ||
      toNumber(pick(totalsObj, 'last7'), NaN) ||
      toNumber(pick(root, 'last_7_days')),
  };
  ...
  // tops (accept snake_case, camelCase, or nested top.{queries,pages,chunks})
  const top = pickObj(root, 'top');
  const tqRaw =
    pickArr(root, 'top_queries').length > 0
      ? pickArr(root, 'top_queries')
      : pickArr(root, 'topQueries').length > 0
      ? pickArr(root, 'topQueries')
      : pickArr(top, 'queries');

  const tpRaw =
    pickArr(root, 'top_pages').length > 0
      ? pickArr(root, 'top_pages')
      : pickArr(root, 'topPages').length > 0
      ? pickArr(root, 'topPages')
      : pickArr(top, 'pages');

  const tcRaw =
    pickArr(root, 'top_chunks').length > 0
      ? pickArr(root, 'top_chunks')
      : pickArr(root, 'topChunks').length > 0
      ? pickArr(root, 'topChunks')
      : pickArr(top, 'chunks');
  ...
  const mapTop = (arr: unknown[]): TopItem[] => {
    ...
    for (const item of arr) {
      if (!isObj(item)) continue;
      const key =
        toStringSafe(pick(item, 'key')) ||
        toStringSafe(pick(item, 'id')) ||
        toStringSafe(pick(item, 'name'));
      const count = toNumber(pick(item, 'count') ?? pick(item, 'value'));
